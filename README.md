<!-- Header -->
<h1 align="center">👋 Hi, I'm Kim Hyun-Bin</h1>
<p align="center">
Recommendation System · AI · NLP 연구에 관심이 있는 대학생입니다.  
데이터 기반 문제 해결과 ML/DL 실험을 즐기며, 다양한 프로젝트를 진행하고 있습니다.
</p>

---

## 🧰 Tech Stack

### 🔹 Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-007396?style=flat&logo=OpenJDK&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-003B57?style=flat&logo=postgresql&logoColor=white)

### 🔹 Machine Learning / Deep Learning
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![scikit-learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat&logo=scikitlearn&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-0095D5?style=flat)
![CatBoost](https://img.shields.io/badge/CatBoost-FFCC00?style=flat)

### 🔹 NLP / Embedding / Text Processing
![KoBERT](https://img.shields.io/badge/KoBERT-512BD4?style=flat)
![Word2Vec](https://img.shields.io/badge/Word2Vec-3A75C4?style=flat)
![Transformers](https://img.shields.io/badge/Transformers-FFD21E?style=flat&logo=HuggingFace&logoColor=black)
![MeCab](https://img.shields.io/badge/MeCab-1F8ACB?style=flat)

### 🔹 Tools
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-0078D4?style=flat&logo=visual-studio-code&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![macOS](https://img.shields.io/badge/macOS-000000?style=flat&logo=apple&logoColor=white)

---

## 🚀 Featured Projects
** 추후 Project_Archive Repository 공개 예정

### 🔹 **Semantic Embedding-Based Recommendation System**
KoSBERT 기반 의미 임베딩 공간을 구축하고, 신어(Neologism)가 추천 시스템의 의미 공간에 미치는 영향을 탐구한 프로젝트.

---

### 🔹 **AI vs Human Text Classification**
Perplexity filtering, Zipf’s law, back-translation 등 NLP 기반 전처리 기법과 XGBoost/CatBoost/RF 기반 앙상블 모델을 활용한 텍스트 분류 프로젝트.

---

### 🔹 **Movie Recommendation System**
사용자 평점 기반 협업 필터링 + 콘텐츠 기반 추천을 결합한 **하이브리드 영화 추천 시스템** 프로젝트.  
- 유저-아이템 매트릭스 기반 예측  
- 영화 메타데이터 TF-IDF / Word2Vec 기반 유사도 계산  

---

## 📊 GitHub Stats

<p align="center">
  <img height="160px" src="https://github-readme-stats-git-masterorgs-github-readme-stats-team.vercel.app/api?username=hyunbini&show_icons=true" />
  <img height="160px" src="https://github-readme-stats-git-masterorgs-github-readme-stats-team.vercel.app/api/top-langs/?username=hyunbini&layout=compact" />
</p>

---

## 🌱 About Me

- Recommendation System · ML/DL 중심으로 공부하고 있습니다.  
- 한국어 텍스트의 의미 구조를 이해하고 모델링하는 과정에 큰 흥미를 느낍니다. 

---

## 📫 Contact

- **GitHub**: https://github.com/hyunbini  

---

<p align="center">✨ Thank you for visiting my profile! ✨</p>
